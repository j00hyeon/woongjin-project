...
## 회고
- 카테고리 테이블을 별도로 분리하여 관리하는 방안을 검토했으나, 과제에서는 Product 테이블 내 category 컬럼으로 관리했습니다.
  - 상품 등록 시 직접 문자열로 입력하는 방식은 오타나 표현 방식이 제각각일 수 있어 일관성이 떨어질 수 있다고 생각했습니다.
  - 카테고리를 별도 테이블로 관리하면, 미리 정의된 카테고리 중 하나를 선택하도록 제한할 수 있어 데이터의 일관성을 유지하고 중복을 방지할 수 있습니다.
  - 과제의 주제가 리팩토링 중심이었기 때문에, 기능 개선에 우선순위를 두었습니다.
- 초반에 null 가능성이나 예외 상황에 대한 고려와 계층 간 책임을 명확히 분리하고 시작하는 것의 중요성을 느꼈습니다.
